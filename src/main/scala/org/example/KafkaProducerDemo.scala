...
object KafkaProducerDemo {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 指定请求的kafka集群列表
    prop.put("bootstrap.servers", "192.168.3.41:9092") // 指定响应方式
    //prop.put("acks", "0")
    prop.put("acks", "all")
    // 请求失败重试次数
    //prop.put("retries", "3")
...
//    同步发送消息
    for (i <- 21 to 40) {
      val rmd = producer.send(new ProducerRecord[String, String]("test2", "hello" + i, "value:" + i)).get()
...
//    异步发送消息
//    for (i <- 0 to 21) {
//      val producerRecord = new ProducerRecord[String,String]("test2", "hello:" + i, "value:" + i)
